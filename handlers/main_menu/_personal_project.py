...
async def hint_about_link(msg: types.Message):
    await msg.answer('Попросите ссылку на у исполнителя')


@dp.message_handler(DeepLinkPrefix(Prefixes.OFFER_PROJECT_))
async def entry_create_post(msg: types.Message, payload: str):
    return States, {'worker_id': int(payload)}  # входим в диалог
